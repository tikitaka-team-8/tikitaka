-- S08 Retry·DLT 적용 후 테스트 - Ticketing DB 재시도 실패 주입
-- 짝수 번호의 테스트 Payment 이벤트 5건만 트랜잭션 직렬화 실패로 처리합니다.
...
CREATE FUNCTION raise_s08_retry_failure_reservation_inbox()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
    IF NEW.event_id IN (
        ...
        RAISE EXCEPTION 'S08 retryable Ticketing DB failure: event_id=%', NEW.event_id
            USING ERRCODE = '40001';
    END IF;
...
CREATE TRIGGER trg_s08_retry_failure_reservation_inbox
    BEFORE INSERT ON p_reservation_inbox
    FOR EACH ROW
    EXECUTE FUNCTION raise_s08_retry_failure_reservation_inbox();
