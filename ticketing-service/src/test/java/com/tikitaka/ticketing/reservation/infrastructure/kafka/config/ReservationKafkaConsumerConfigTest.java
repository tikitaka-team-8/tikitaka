...
class ReservationKafkaConsumerConfigTest {
    ...
    private ReservationKafkaConsumerConfig config;
    private ConsumerRecord<String, String> record;

    @BeforeEach
    void setUp() {
        config = new ReservationKafkaConsumerConfig();
        record = new ConsumerRecord<>(KafkaTopics.PAYMENT_EVENTS, 0, 10L, "reservation-id", "payload");
    }

    @Test
    void 재시도는_2초_간격으로_2회_수행한다() {
        // given
        BackOffExecution execution = config.reservationRetryBackOff().start();
    ...
    @Test
    void Payment_이벤트의_DLT는_원본과_동일한_파티션을_사용한다() {
        // given
