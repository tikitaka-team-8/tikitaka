...
public class NotificationKafkaConsumerConfig {
    ...
    // Spring Boot 공통 Consumer 설정을 적용하고 Notification 전용 오류 처리 정책을 연결
    @Bean
    public ConcurrentKafkaListenerContainerFactory<Object, Object> notificationKafkaListenerContainerFactory(
        ...
        factory.setCommonErrorHandler(notificationKafkaErrorHandler(kafkaTemplate));

        return factory;
    }

    private DefaultErrorHandler notificationKafkaErrorHandler(KafkaTemplate<Object, Object> kafkaTemplate) {
    ...
    TopicPartition resolveDltDestination(ConsumerRecord<?, ?> record, Exception exception) {
        ...
        log.error(
                "Reservation 이벤트 최종 처리 실패로 DLT 전송: topic={}, partition={}, offset={}, dltTopic={}",
                record.topic(), record.partition(), record.offset(), KafkaTopics.RESERVATION_EVENTS_DLT, exception
        );

        return new TopicPartition(KafkaTopics.RESERVATION_EVENTS_DLT, record.partition());
    }
}
